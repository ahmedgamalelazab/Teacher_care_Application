...
class Moderator_Screen extends StatefulWidget {
  static const String SCREEN_ROUTE = '/Moderator_Screen';
  const Moderator_Screen({Key? key}) : super(key: key);

  @override
  _Moderator_ScreenState createState() => _Moderator_ScreenState();
}

class _Moderator_ScreenState extends State<Moderator_Screen> {
  dynamic userState;
  int begin = 0;
...
_display_proper_screen(
  ...
  switch (currentIndex) {
    case 0:
      return Center(
        child: Text('Moderator profile'),
      );
    case 1:
      return Center(
        child: Text('Moderator Messages'),
      );
    case 2:
      return Center(
        child: Text('Moderator settings'),
      );
    default:
  }
}
